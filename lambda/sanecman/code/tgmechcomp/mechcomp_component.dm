...
#define MECHCORP_RADMENU_ICONFILE 'lambda/sanecman/icons/mechcomp/connection.dmi'
...
/datum/component/mechanics_holder/proc/dropConnect(var/comsig_target, obj/over, mob/user)
	...
	*/
	...
	//typesel += list("Trigger") = image(icon = 'lambda/sanecman/icons/mechcomp/connection.dmi', icon_state = "trigger")
	//typesel += list("Receiver") = image(icon = 'lambda/sanecman/icons/mechcomp/connection.dmi', icon_state = "receiver")
...
/datum/component/mechanics_holder/proc/attackby(var/comsig_target, obj/item/W /*as obj*/ /*НАХУЯ???*/, mob/user)
	...
	if(length(src.configs))
		...
		for(var/i in src.configs)
			...
			switch(i)
				if(SET_SEND)
					img = image(icon = 'lambda/sanecman/icons/mechcomp/connection.dmi', icon_state = "signal")
				if(DC_ALL)
					img = image(icon = 'lambda/sanecman/icons/mechcomp/connection.dmi', icon_state = "disconnect")
				else
					img = image(icon = 'lambda/sanecman/icons/mechcomp/connection.dmi', icon_state = "config")
			img.maptext_width = 64
			img.maptext_x = -16
			img.maptext = "<text align=center valign=bottom>[MAPTEXT("[i]")]"
			choices.Add("[i]")
		...
		if(selected_config && in_range(parent, user))
			switch(selected_config)
				if(SET_SEND)
					var/inp = input(user,"Please enter Signal:","Signal setting", "[defaultSignal]")
