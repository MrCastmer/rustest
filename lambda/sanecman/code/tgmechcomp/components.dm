...
/obj/item/mechcomp/textpad/interact_by_hand()
	var/inp = input(usr, "What text would you like to input?", "Oh, the possibilities!", null)
	if(isnull(inp))
		return
...
/obj/item/mechcomp/list_packer/proc/prompt_update(var/varname, var/v)
	var/input = input(usr, "Set [varname] to what? Careful, empty input will erase what's currently stored in [varname]!", "[varname]", v)
	return input
...
/obj/item/mechcomp/list_packer/proc/set_glue(obj/item/I, mob/user)
	var/input = input(user, "Set glue to what? Glue is used to \"glue\" lists together into a single string. Default glue for most cases is \"&\", but you can use another one if you want to use lists of lists. You can even use multiple symbols as glue!", "glue", glue)
	if(!isnull(input))
		glue = input
...
/obj/item/mechcomp/list_extractor/proc/set_glue(obj/item/I, mob/user)
	var/input = input(user, "Set glue to what? Glue is used to \"glue\" lists together into a single string. Default glue for most cases is \"&\", but you can use another one if you want to use lists of lists. You can even use multiple symbols as glue! Make sure the list you pass to [src.name] uses the same glue!", "Glue", glue)
	if(!isnull(input))
		glue = input
		to_chat(user, span_notice("You set [src.name]'s glue to \"[glue]\""))
...
/obj/item/mechcomp/find_regex/proc/setpattern(obj/item/I, mob/user)
	var/input = input(user, "Input your regex pattern.", "[pick(80;"Regex", 5;"Reg-ekhs?", 5;"what the fuck is a regex", 5;"if you have a problem you want to solve with regex, you have 2 problems.", 5;"the regex is outlawed in 48 US states")]", reg?.name)
	if(!isnull(input))
...
/obj/item/mechcomp/find_regex/proc/set_group_glue(obj/item/I, mob/user)
	var/input = input(user, "Set group glue to what? Group glue for regex is used to glue together all the capture groups from the single search. It is heavily recommended to keep different from result glue which is used to glue together all search results.", "Glue", glue)
	if(!isnull(input))
		glue = input
